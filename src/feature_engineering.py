...
def load_data(path: str):
    """
    Load the raw credit default dataset from a specified file path.
...
def feature_engineering(df: pd.DataFrame):
    """
    Apply bank-style feature engineering to the credit default dataset.
    
    This function generates behavioral, utilization, stability, and payment 
    capacity features, imputes missing values, and prunes highly correlated columns.

    Args:
        df (pd.DataFrame): The cleaned credit default dataframe.

    Returns:
        pd.DataFrame: A new dataframe containing the engineered features.
    """
    df = df.copy()
...
if __name__ == "__main__":
    INPUT_PATH = "data/processed/credit_default_cleaned.csv"
    OUTPUT_PATH = "data/processed/credit_default_engineered.csv"
